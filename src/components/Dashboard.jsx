import React, { useState } from "react";
import { Link, Outlet } from "react-router-dom";
import { Bars3Icon } from "@heroicons/react/24/outline";
const Dashboard = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [isSideMenuOpen, setIsSideMenuOpen] = useState(false);

  const toggleSideMenu = () => setIsSideMenuOpen(!isSideMenuOpen);

  const menuItems = [
    {
      text: "Assistants",
      path: "/assistants",
      icon: (
        <svg
          width="23.3887"
          height="17.9785"
          viewBox="0 0 23.3887 17.9785"
          fill="'none' || '#000000"
          stroke="none"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out !fill-icon/30 !text-text/40"
        >
          <g>
            <rect
              height="17.9785"
              opacity="0"
              width="23.3887"
              x="0"
              y="0"
            ></rect>
            <path
              d="M3.06641 17.9785L19.9609 17.9785C22.0117 17.9785 23.0273 16.9727 23.0273 14.9609L23.0273 3.02734C23.0273 1.01562 22.0117 0 19.9609 0L3.06641 0C1.02539 0 0 1.01562 0 3.02734L0 14.9609C0 16.9727 1.02539 17.9785 3.06641 17.9785ZM3.08594 16.4062C2.10938 16.4062 1.57227 15.8887 1.57227 14.873L1.57227 3.11523C1.57227 2.09961 2.10938 1.57227 3.08594 1.57227L19.9414 1.57227C20.9082 1.57227 21.4551 2.09961 21.4551 3.11523L21.4551 14.873C21.4551 15.8887 20.9082 16.4062 19.9414 16.4062ZM4.80469 16.9922L18.2422 16.9922C17.666 14.248 14.9121 12.2559 11.5234 12.2559C8.125 12.2559 5.37109 14.248 4.80469 16.9922ZM11.5234 10.625C13.3887 10.6445 14.8633 9.05273 14.8633 6.96289C14.8633 5 13.3887 3.37891 11.5234 3.37891C9.6582 3.37891 8.17383 5 8.18359 6.96289C8.19336 9.05273 9.6582 10.6055 11.5234 10.625Z"
              fill-opacity="0.85"
            ></path>
          </g>
        </svg>
      ),
    },
    {
      text: "Phone Numbers",
      path: "/phone-numbers",
      icon: (
        <svg
          width="18.4668"
          height="18.8379"
          viewBox="0 0 18.4668 18.8379"
          fill="'none' || '#000000"
          stroke="none"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out !fill-icon/30 !text-text/40"
        >
          <g>
            <rect
              height="18.8379"
              opacity="0"
              width="18.4668"
              x="0"
              y="0"
            ></rect>
            <path
              d="M4.96094 13.8379C7.7832 16.6602 11.2109 18.8379 14.0137 18.8379C15.2734 18.8379 16.377 18.3984 17.2656 17.4219C17.7832 16.8457 18.1055 16.1719 18.1055 15.5078C18.1055 15.0195 17.9199 14.5508 17.4609 14.2188L14.4629 12.0898C14.0137 11.7773 13.623 11.6211 13.2715 11.6211C12.832 11.6211 12.4316 11.875 11.9824 12.3145L11.2891 12.998C11.1816 13.1055 11.0449 13.1543 10.918 13.1543C10.7715 13.1543 10.625 13.0957 10.5273 13.0469C9.92188 12.7246 8.88672 11.8359 7.91992 10.8789C6.96289 9.92188 6.07422 8.88672 5.76172 8.28125C5.71289 8.17383 5.6543 8.03711 5.6543 7.89062C5.6543 7.76367 5.69336 7.63672 5.80078 7.5293L6.49414 6.81641C6.92383 6.36719 7.1875 5.97656 7.1875 5.52734C7.1875 5.17578 7.03125 4.79492 6.69922 4.33594L4.59961 1.37695C4.26758 0.908203 3.7793 0.703125 3.25195 0.703125C2.60742 0.703125 1.94336 0.996094 1.36719 1.55273C0.419922 2.46094 0 3.58398 0 4.82422C0 7.62695 2.13867 11.0254 4.96094 13.8379Z"
              fill-opacity="0.85"
            ></path>
          </g>
        </svg>
      ),
    },
    {
      text: "Files",
      path: "/files",
      icon: (
        <svg
          width="16.7871"
          height="20.9473"
          viewBox="0 0 16.7871 20.9473"
          fill="'none' || '#000000"
          stroke="none"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out fill-primary !text-text"
        >
          <g>
            <rect
              height="20.9473"
              opacity="0"
              width="16.7871"
              x="0"
              y="0"
            ></rect>
            <path
              d="M4.38477 13.2617C4.00391 13.2617 3.7207 12.9883 3.7207 12.627C3.7207 12.2656 4.00391 11.9824 4.38477 11.9824L12.0703 11.9824C12.4414 11.9824 12.7246 12.2656 12.7246 12.627C12.7246 12.9883 12.4414 13.2617 12.0703 13.2617ZM4.38477 16.9629C4.00391 16.9629 3.7207 16.6895 3.7207 16.3281C3.7207 15.9668 4.00391 15.6836 4.38477 15.6836L12.0703 15.6836C12.4414 15.6836 12.7246 15.9668 12.7246 16.3281C12.7246 16.6895 12.4414 16.9629 12.0703 16.9629ZM3.02734 20.9375L13.3984 20.9375C15.4199 20.9375 16.4258 19.9121 16.4258 17.8809L16.4258 9.01367L9.35547 9.01367C8.10547 9.01367 7.50977 8.41797 7.50977 7.16797L7.50977 0L3.02734 0C1.01562 0 0 1.03516 0 3.06641L0 17.8809C0 19.9219 1.00586 20.9375 3.02734 20.9375ZM9.38477 7.67578L16.3086 7.67578C16.25 7.27539 15.9668 6.88477 15.498 6.40625L10.1172 0.927734C9.6582 0.458984 9.25781 0.175781 8.84766 0.107422L8.84766 7.14844C8.84766 7.5 9.02344 7.67578 9.38477 7.67578Z"
              fill-opacity="0.85"
            ></path>
          </g>
        </svg>
      ),
    },
    {
      text: "Tools",
      path: "/tools",
      icon: (
        <svg
          width="23.5718"
          height="23.4864"
          viewBox="0 0 23.5718 23.4864"
          fill="'none' || '#000000"
          stroke="none"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out fill-primary !text-text"
        >
          <g>
            <rect
              height="23.4864"
              opacity="0"
              width="23.5718"
              x="0"
              y="0"
            ></rect>
            <path
              d="M18.3728 17.3438L19.408 17.3926C19.7303 17.4024 20.0623 17.6172 20.2381 17.9395L22.0056 21.1914C22.1717 21.5039 22.2107 21.7383 22.0252 21.9239L20.8826 23.0664C20.6873 23.2618 20.4529 23.2129 20.1599 23.0664L16.8885 21.2793C16.5564 21.1133 16.3513 20.7715 16.3416 20.4688L16.2928 19.4336L11.9221 15.063C12.4574 14.3846 13.0875 13.6401 13.8387 12.8184ZM6.4783 2.08012L12.3045 7.91459C12.3615 8.07841 12.4287 8.23969 12.5037 8.39848C10.0741 10.7952 8.21996 12.2488 6.74001 13.2386L1.02908 7.51957C-0.338105 6.15239-0.338105 4.68754 0.999786 3.34965L2.29861 2.05082C3.6365 0.712933 5.10135 0.712933 6.4783 2.08012ZM2.99197 5.32231C2.84549 5.48832 2.83572 5.73246 2.99197 5.88871L6.86893 9.74614C7.02518 9.90239 7.27908 9.90239 7.42557 9.74614C7.57205 9.59965 7.58182 9.36528 7.42557 9.20903L3.53885 5.32231C3.40213 5.19536 3.14822 5.17582 2.99197 5.32231ZM4.28104 4.05278C4.12479 4.20903 4.13455 4.4434 4.28104 4.58989L8.14822 8.48637C8.29471 8.64262 8.56814 8.62309 8.6951 8.48637C8.85135 8.31059 8.85135 8.06645 8.70486 7.91996L4.82791 4.05278C4.67166 3.89653 4.41775 3.90629 4.28104 4.05278Z"
              fill-opacity="0.85"
            ></path>
            <path
              d="M1.52713 22.2168C3.22635 23.8965 5.50174 23.9161 7.17166 22.2364C9.27127 20.1172 8.93924 17.7344 15.3455 11.2305C17.4353 12.2168 19.9646 11.8848 21.6443 10.2149C23.4021 8.45707 23.7049 5.68364 22.3963 3.43754L19.74 6.09379C19.4763 6.35746 19.1443 6.377 18.8611 6.09379L17.6697 4.89262C17.3865 4.59965 17.3572 4.25786 17.6209 3.99418L20.2869 1.3477C18.0603 0.0488707 15.2967 0.341839 13.5388 2.10942C11.8592 3.78911 11.5174 6.30864 12.5037 8.39848C6.00955 14.8047 3.62674 14.4727 1.5076 16.5625C-0.181855 18.2325-0.162324 20.5176 1.52713 22.2168ZM14.5935 9.15043C12.8553 7.41215 12.7869 4.65825 14.4471 3.01762C15.3846 2.07036 16.7224 1.6895 18.0017 1.89457L16.5857 3.31059C15.8631 4.02348 15.8826 4.94145 16.6346 5.67387L18.0799 7.10942C18.8025 7.83207 19.7303 7.85161 20.4138 7.15825L21.8494 5.71293C22.0642 7.01176 21.6736 8.34965 20.7263 9.30668C19.076 10.9571 16.3416 10.8789 14.5935 9.15043ZM2.49393 21.25C1.37088 20.1075 1.39041 18.6524 2.46463 17.5684C4.82791 15.2247 6.23416 16.2207 13.158 9.45317C13.3142 9.66801 13.49 9.86332 13.6756 10.0586C13.8611 10.2442 14.0662 10.4297 14.281 10.586C7.52322 17.5098 8.51932 18.9161 6.17557 21.2598C5.09158 22.3536 3.62674 22.3633 2.49393 21.25ZM4.31033 20.6641C4.97439 20.6641 5.52127 20.1172 5.52127 19.4532C5.52127 18.7891 4.97439 18.252 4.31033 18.252C3.64627 18.252 3.10916 18.7891 3.10916 19.4532C3.10916 20.1172 3.64627 20.6641 4.31033 20.6641Z"
              fill-opacity="0.85"
            ></path>
          </g>
        </svg>
      ),
    },
    {
      text: "Blocks",
      path: "/blocks",
      icon: (
        <svg
          width="21.3574"
          height="23.1299"
          viewBox="0 0 21.3574 23.1299"
          fill="'none' || '#000000"
          stroke="none"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out fill-primary !text-text"
        >
          <g>
            <rect
              height="23.1299"
              opacity="0"
              width="21.3574"
              x="0"
              y="0"
            ></rect>
            <path
              d="M2.33398 8.21045L18.6621 8.21045C20.127 8.21045 20.9961 7.35107 20.9961 5.91553L20.9961 2.61475C20.9961 1.1792 20.127 0.319824 18.6621 0.319824L2.33398 0.319824C0.869141 0.319824 0 1.1792 0 2.61475L0 5.91553C0 7.35107 0.869141 8.21045 2.33398 8.21045ZM2.36328 6.73584C1.80664 6.73584 1.47461 6.39404 1.47461 5.82764L1.47461 2.69287C1.47461 2.13623 1.80664 1.79443 2.36328 1.79443L18.6328 1.79443C19.1895 1.79443 19.5215 2.13623 19.5215 2.69287L19.5215 5.82764C19.5215 6.39404 19.1895 6.73584 18.6328 6.73584ZM9.79492 10.5347L11.2207 10.5347L11.2207 7.44873L9.79492 7.44873ZM2.33398 17.5073L18.6621 17.5073C20.127 17.5073 20.9961 16.6479 20.9961 15.2124L20.9961 11.9116C20.9961 10.4761 20.127 9.6167 18.6621 9.6167L2.33398 9.6167C0.869141 9.6167 0 10.4761 0 11.9116L0 15.2124C0 16.6479 0.869141 17.5073 2.33398 17.5073ZM2.36328 16.0327C1.80664 16.0327 1.47461 15.6909 1.47461 15.1343L1.47461 11.9995C1.47461 11.4331 1.80664 11.0913 2.36328 11.0913L18.6328 11.0913C19.1895 11.0913 19.5215 11.4331 19.5215 11.9995L19.5215 15.1343C19.5215 15.6909 19.1895 16.0327 18.6328 16.0327ZM9.78516 19.5483L11.2207 19.5483L11.2207 16.4722L9.78516 16.4722ZM9.98047 22.8589C10.2734 23.2202 10.7617 23.2202 11.0547 22.8589L13.291 20.0952C13.7207 19.5581 13.457 18.855 12.7637 18.855L8.26172 18.855C7.54883 18.855 7.27539 19.5483 7.72461 20.0952Z"
              fill-opacity="0.85"
            ></path>
          </g>
        </svg>
      ),
    },
    {
      text: "Squads",
      path: "/squads",
      icon: (
        <svg
          width="23.9648"
          height="24.7363"
          viewBox="0 0 23.9648 24.7363"
          fill="'none' || '#000000"
          stroke="none"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out !fill-icon/30 !text-text/40"
        >
          <g>
            <rect
              height="24.7363"
              opacity="0"
              width="23.9648"
              x="0"
              y="0"
            ></rect>
            <path
              d="M17.9647 2.79297L5.63882 2.79297C5.6789 1.99929 6.17978 1.54297 7.01172 1.54297L16.5918 1.54297C17.4237 1.54297 17.9246 1.99929 17.9647 2.79297Z"
              fill-opacity="0.85"
            ></path>
            <path
              d="M19.9623 5.5825C19.6482 5.53257 19.3151 5.50781 18.9648 5.50781L4.63867 5.50781C4.2885 5.50781 3.95544 5.53256 3.64145 5.58247C3.74778 4.64426 4.38263 4.12109 5.41992 4.12109L18.1934 4.12109C19.2307 4.12109 19.8575 4.64428 19.9623 5.5825Z"
              fill-opacity="0.85"
            ></path>
            <path
              d="M5.70312 22.373L17.9004 22.373C17.3828 19.8828 14.8828 18.0762 11.8066 18.0762C8.7207 18.0762 6.2207 19.8828 5.70312 22.373ZM11.8066 16.5918C13.4961 16.5918 14.834 15.1758 14.834 13.2812C14.834 11.4941 13.4961 10.0293 11.8066 10.0293C10.1074 10.0293 8.7793 11.4941 8.7793 13.2812C8.7793 15.1758 10.1074 16.5918 11.8066 16.5918ZM4.63867 23.1641L18.9648 23.1641C21.0059 23.1641 22.0312 22.1484 22.0312 20.1367L22.0312 10.1074C22.0312 8.0957 21.0059 7.08008 18.9648 7.08008L4.63867 7.08008C2.59766 7.08008 1.57227 8.0957 1.57227 10.1074L1.57227 20.1367C1.57227 22.1484 2.59766 23.1641 4.63867 23.1641ZM4.66797 21.5918C3.69141 21.5918 3.14453 21.0645 3.14453 20.0488L3.14453 10.1953C3.14453 9.17969 3.69141 8.65234 4.66797 8.65234L18.9453 8.65234C19.9023 8.65234 20.459 9.17969 20.459 10.1953L20.459 20.0488C20.459 21.0645 19.9023 21.5918 18.9453 21.5918Z"
              fill-opacity="0.85"
            ></path>
          </g>
        </svg>
      ),
    },
  ];
  return (
    <div className="flex h-screen">
      {/* Sidebar */}
      <div className="z-20 hidden w-60 overflow-y-auto bg-gray-900 md:flex md:flex-col">
        <div className="flex flex-col flex-1">
          <div className="text-white">
            <div className="flex p-2">
              <div className="flex items-center py-3 px-2">
                <p className="ml-2 font-semibold">DASHBOARD</p>
              </div>
            </div>
            <div>
              <ul className="mt-6 leading-10">
                <li className="relative px-2 py-1 ml-2">
                  <Link to="/overview" className="group">
                    <p className="inline-flex items-center w-full px-1 py-1 text-sm font-semibold text-white transition-colors duration-150 cursor-pointer hover:bg-pink-700 rounded-md">
                      <span className="rounded-md bg-gray-100  group-hover:bg-pink-100 px-1 py-1 text-gray-700 text-center">
                        <svg
                          width="20.2832"
                          height="19.9121"
                          viewBox="0 0 20.2832 19.9121"
                          fill="'none' || '#000000"
                          stroke="none"
                          stroke-width="2"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out !fill-icon/30 !text-text/40"
                        >
                          <g>
                            <rect
                              height="19.9121"
                              opacity="0"
                              width="20.2832"
                              x="0"
                              y="0"
                            ></rect>
                            <path
                              d="M9.96094 19.9121C12.3047 19.9121 14.5703 19.0723 16.377 17.5391L9.3457 10.6445C9.15039 10.4688 9.11133 10.3027 9.11133 10.0391L9.11133 0.0292969C4.04297 0.46875 0 4.79492 0 9.95117C0 15.4004 4.52148 19.9121 9.96094 19.9121ZM19.9219 9.95117C19.9219 8.48633 19.5605 6.95312 18.9453 5.70312L10.9668 10.293L17.3438 16.5918C18.9453 14.8438 19.9219 12.4414 19.9219 9.95117ZM10.4199 9.0918L18.3008 4.56055C16.6016 1.89453 13.5254 0.126953 10.4199 0Z"
                              fill-opacity="0.85"
                            ></path>
                          </g>
                        </svg>
                      </span>
                      <span className="ml-4 text-base">Overview</span>
                    </p>
                  </Link>
                </li>
                <li className="relative px-2 py-1">
                  <div
                    className=" ml-2 inline-flex items-center justify-between w-full px-1 py-1 text-base font-semibold transition-colors duration-150 cursor-pointer text-gray-500 hover:text-yellow-400 hover:bg-pink-700 rounded-md group"
                    onClick={() => setIsOpen(!isOpen)}
                  >
                    <p className="inline-flex items-center text-base font-semibold text-white  ">
                      <span className="rounded-md  bg-gray-100  group-hover:bg-pink-100  px-1 py-1  text-center">
                        <svg
                          width="20.0098"
                          height="19.6582"
                          viewBox="0 0 20.0098 19.6582"
                          fill="'none' || '#000000"
                          stroke="none"
                          stroke-width="2"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          class="fill-icon/30 group-hover:fill-icon/80 w-[14px] h-[14px] transition-all duration-150 ease-in-out"
                        >
                          <g>
                            <rect
                              height="19.6582"
                              opacity="0"
                              width="20.0098"
                              x="0"
                              y="0"
                            ></rect>
                            <path
                              d="M0 13.0664C0 13.5352 0.3125 13.8574 0.78125 13.8574L1.78711 13.8574L1.78711 17.0996C1.78711 17.5684 2.09961 17.8711 2.56836 17.8711L5.80078 17.8711L5.80078 18.877C5.80078 19.3457 6.12305 19.6582 6.5918 19.6582C7.06055 19.6582 7.37305 19.3457 7.37305 18.877L7.37305 17.8711L9.04297 17.8711L9.04297 18.877C9.04297 19.3457 9.35547 19.6582 9.82422 19.6582C10.293 19.6582 10.6152 19.3457 10.6152 18.877L10.6152 17.8711L12.2754 17.8711L12.2754 18.877C12.2754 19.3457 12.5879 19.6582 13.0566 19.6582C13.5352 19.6582 13.8477 19.3457 13.8477 18.877L13.8477 17.8711L17.0801 17.8711C17.5488 17.8711 17.8613 17.5684 17.8613 17.0996L17.8613 13.8477L18.8672 13.8477C19.3359 13.8477 19.6484 13.5254 19.6484 13.0566C19.6484 12.5879 19.3359 12.2754 18.8672 12.2754L17.8613 12.2754L17.8613 10.6055L18.8672 10.6055C19.3359 10.6055 19.6484 10.293 19.6484 9.82422C19.6484 9.35547 19.3359 9.0332 18.8672 9.0332L17.8613 9.0332L17.8613 7.37305L18.8672 7.37305C19.3359 7.37305 19.6484 7.05078 19.6484 6.58203C19.6484 6.11328 19.3359 5.80078 18.8672 5.80078L17.8613 5.80078L17.8613 2.57812C17.8613 2.10938 17.5488 1.79688 17.0801 1.79688L13.8477 1.79688L13.8477 0.800781C13.8477 0.332031 13.5352 0.00976562 13.0566 0.00976562C12.5879 0.00976562 12.2754 0.332031 12.2754 0.800781L12.2754 1.79688L10.6152 1.79688L10.6152 0.800781C10.6152 0.332031 10.293 0.00976562 9.82422 0.00976562C9.35547 0.00976562 9.04297 0.332031 9.04297 0.800781L9.04297 1.79688L7.37305 1.79688L7.37305 0.800781C7.37305 0.332031 7.06055 0.00976562 6.5918 0.00976562C6.12305 0.00976562 5.80078 0.332031 5.80078 0.800781L5.80078 1.79688L2.56836 1.79688C2.09961 1.79688 1.78711 2.10938 1.78711 2.57812L1.78711 5.81055L0.78125 5.81055C0.3125 5.81055 0 6.12305 0 6.5918C0 7.06055 0.3125 7.38281 0.78125 7.38281L1.78711 7.38281L1.78711 9.04297L0.78125 9.04297C0.3125 9.04297 0 9.36523 0 9.83398C0 10.3027 0.3125 10.6152 0.78125 10.6152L1.78711 10.6152L1.78711 12.2852L0.78125 12.2852C0.3125 12.2852 0 12.5977 0 13.0664ZM6.09375 14.3457C5.57617 14.3457 5.32227 14.1016 5.32227 13.5645L5.32227 6.10352C5.32227 5.57617 5.57617 5.33203 6.09375 5.33203L13.5742 5.33203C14.0918 5.33203 14.3359 5.57617 14.3359 6.10352L14.3359 13.5645C14.3359 14.1016 14.0918 14.3457 13.5742 14.3457ZM6.65039 12.7734C6.65039 12.9199 6.74805 13.0176 6.89453 13.0176L12.7637 13.0176C12.9199 13.0176 13.0078 12.9199 13.0078 12.7734L13.0078 6.9043C13.0078 6.74805 12.9199 6.66016 12.7637 6.66016L6.89453 6.66016C6.74805 6.66016 6.65039 6.74805 6.65039 6.9043Z"
                              fill-opacity="0.85"
                            ></path>
                          </g>
                        </svg>
                      </span>
                      <span className="ml-4 text-base">Platform</span>
                    </p>
                    {isOpen ? (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="ml-1 text-white w-4 h-4"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M19 9l-7 7-7-7"
                        />
                      </svg>
                    ) : (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="ml-1 text-white w-4 h-4"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          strokeWidth="2"
                          d="M15 19l-7-7 7-7"
                        />
                      </svg>
                    )}
                  </div>
                  {isOpen && (
                    <div>
                      <ul
                        className="p-2 space-y-2 overflow-hidden text-sm font-medium flex flex-col rounded-md shadow-inner h-full"
                        aria-label="submenu"
                      >
                        {menuItems.map((item, index) => (
                          <li
                            key={index}
                            className="px-2 py-1 text-white transition-colors duration-150 group"
                          >
                            <div className="px-1 py-1 hover:text-white hover:bg-pink-700 rounded-md ">
                              <div className="flex items-center">
                                <span className="rounded-md bg-gray-100  group-hover:bg-pink-100 p-1 text-gray-700 text-center">
                                  {item.icon}
                                </span>
                                <Link
                                  to={item.path}
                                  onClick={() => setIsOpen(false)}
                                >
                                  <p className="w-full ml-2 text-base font-semibold text-white">
                                    {item.text}
                                  </p>
                                </Link>
                              </div>
                            </div>
                          </li>
                        ))}
                      </ul>
                    </div>
                  )}
                </li>
                <li className=" ml-2 px-2 py-1 text-white transition-colors duration-150 group">
                  <div className="px-1 py-1 hover:bg-pink-700 rounded-md">
                    <div className="flex items-center">
                      <span className="rounded-md bg-gray-100  group-hover:bg-pink-100 p-1 text-gray-700 text-center">
                        <svg
                          width="18.3594"
                          height="20.4004"
                          viewBox="0 0 18.3594 20.4004"
                          fill="'none' || '#000000"
                          stroke="none"
                          stroke-width="2"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out !fill-icon/30 !text-text/40"
                        >
                          <g>
                            <rect
                              height="20.4004"
                              opacity="0"
                              width="18.3594"
                              x="0"
                              y="0"
                            ></rect>
                            <path
                              d="M17.207 13.3105C17.6465 13.3105 17.998 12.959 17.998 12.5293L17.998 7.87109C17.998 7.44141 17.6465 7.08008 17.207 7.08008C16.7578 7.08008 16.4258 7.44141 16.4258 7.87109L16.4258 12.5293C16.4258 12.959 16.7578 13.3105 17.207 13.3105Z"
                              fill-opacity="0.85"
                            ></path>
                            <path
                              d="M13.9258 18.2324C14.3652 18.2324 14.707 17.8809 14.707 17.4512L14.707 2.94922C14.707 2.51953 14.3652 2.1582 13.9258 2.1582C13.4668 2.1582 13.1348 2.51953 13.1348 2.94922L13.1348 17.4512C13.1348 17.8809 13.4668 18.2324 13.9258 18.2324Z"
                              fill-opacity="0.85"
                            ></path>
                            <path
                              d="M10.6348 15.2734C11.084 15.2734 11.4258 14.9316 11.4258 14.4922L11.4258 5.9082C11.4258 5.46875 11.084 5.11719 10.6348 5.11719C10.1855 5.11719 9.85352 5.46875 9.85352 5.9082L9.85352 14.4922C9.85352 14.9316 10.1855 15.2734 10.6348 15.2734Z"
                              fill-opacity="0.85"
                            ></path>
                            <path
                              d="M7.34375 20.4004C7.79297 20.4004 8.14453 20.0488 8.14453 19.6094L8.14453 0.791016C8.14453 0.351562 7.79297 0 7.34375 0C6.9043 0 6.57227 0.351562 6.57227 0.791016L6.57227 19.6094C6.57227 20.0488 6.9043 20.4004 7.34375 20.4004Z"
                              fill-opacity="0.85"
                            ></path>
                            <path
                              d="M4.0625 16.4648C4.51172 16.4648 4.85352 16.1133 4.85352 15.6738L4.85352 4.72656C4.85352 4.28711 4.51172 3.92578 4.0625 3.92578C3.62305 3.92578 3.28125 4.28711 3.28125 4.72656L3.28125 15.6738C3.28125 16.1133 3.62305 16.4648 4.0625 16.4648Z"
                              fill-opacity="0.85"
                            ></path>
                            <path
                              d="M0.771484 12.4707C1.23047 12.4707 1.57227 12.1191 1.57227 11.6797L1.57227 8.7207C1.57227 8.28125 1.23047 7.91992 0.771484 7.91992C0.332031 7.91992 0 8.28125 0 8.7207L0 11.6797C0 12.1191 0.332031 12.4707 0.771484 12.4707Z"
                              fill-opacity="0.85"
                            ></path>
                          </g>
                        </svg>
                      </span>
                      <Link to="/voice-library">
                        <p className="w-full ml-2 text-sm font-semibold text-white">
                          Voice Library
                        </p>
                      </Link>
                    </div>
                  </div>
                </li>
                <li className="ml-2 px-2 py-1 text-white transition-colors duration-150 group">
                  <div className="px-1 py-1 hover:bg-pink-700 rounded-md">
                    <div className="flex items-center">
                      <span className="rounded-md bg-gray-100  group-hover:bg-pink-100 p-1 text-gray-700 text-center">
                        <svg
                          width="23.3887"
                          height="16.6895"
                          viewBox="0 0 23.3887 16.6895"
                          fill="'none' || '#000000"
                          stroke="none"
                          stroke-width="2"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out !fill-icon/80 !text-text/80"
                        >
                          <g>
                            <rect
                              height="16.6895"
                              opacity="0"
                              width="23.3887"
                              x="0"
                              y="0"
                            ></rect>
                            <path
                              d="M3.06641 16.6895L19.9609 16.6895C22.0117 16.6895 23.0273 15.6738 23.0273 13.6621L23.0273 8.27148C23.0273 7.39258 22.8906 6.98242 22.4707 6.41602L19.1016 1.85547C18.0371 0.400391 17.4512 0.0195312 15.7129 0.0195312L7.31445 0.0195312C5.57617 0.0195312 5 0.400391 3.93555 1.85547L0.556641 6.41602C0.146484 6.98242 0 7.39258 0 8.27148L0 13.6621C0 15.6836 1.02539 16.6895 3.06641 16.6895ZM11.5137 11.0352C10.0488 11.0352 9.11133 9.76562 9.11133 8.56445L9.11133 8.53516C9.11133 8.0957 8.84766 7.68555 8.30078 7.68555L2.08008 7.68555C1.75781 7.68555 1.68945 7.41211 1.8457 7.19727L5.45898 2.23633C5.9082 1.61133 6.48438 1.38672 7.19727 1.38672L15.8301 1.38672C16.543 1.38672 17.1191 1.61133 17.5684 2.23633L21.1816 7.19727C21.3281 7.41211 21.2695 7.68555 20.9473 7.68555L14.7266 7.68555C14.1797 7.68555 13.9258 8.0957 13.9258 8.53516L13.9258 8.56445C13.9258 9.76562 12.9883 11.0352 11.5137 11.0352ZM15.6152 2.82227L7.41211 2.82227C7.14844 2.82227 6.94336 3.02734 6.94336 3.29102C6.94336 3.55469 7.14844 3.75977 7.41211 3.75977L15.6152 3.75977C15.8789 3.75977 16.084 3.55469 16.084 3.29102C16.084 3.02734 15.8789 2.82227 15.6152 2.82227ZM16.9141 5.0293L6.11328 5.0293C5.83984 5.0293 5.625 5.27344 5.625 5.53711C5.625 5.80078 5.83984 6.02539 6.11328 6.02539L16.9141 6.02539C17.1875 6.02539 17.4023 5.80078 17.4023 5.53711C17.4023 5.27344 17.1875 5.0293 16.9141 5.0293Z"
                              fill-opacity="0.85"
                            ></path>
                          </g>
                        </svg>
                      </span>
                      <Link to="/call-logs">
                        <p className="w-full ml-2 text-sm font-semibold text-white">
                          Call Logs
                        </p>
                      </Link>
                    </div>
                  </div>
                </li>
                <li className="ml-2 px-2 py-1 text-white transition-colors duration-150 absolute bottom-0 group ">
                  <div className="px-1 py-1  hover:bg-pink-700 rounded-md w-56">
                    <div className="flex items-center">
                      <span className="rounded-md bg-gray-100  group-hover:bg-pink-100 p-1 text-gray-700 text-center">
                        <svg
                          width="16.7969"
                          height="17.5684"
                          viewBox="0 0 16.7969 17.5684"
                          fill="'none' || '#000000"
                          stroke="none"
                          stroke-width="2"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          class="w-3.5 h-3.5 transition-all duration-100 ease-in-out fill-primary !text-text"
                        >
                          <g>
                            <rect
                              height="17.5684"
                              opacity="0"
                              width="16.7969"
                              x="0"
                              y="0"
                            ></rect>
                            <path
                              d="M1.66992 17.5586L14.7656 17.5586C15.8105 17.5586 16.4355 17.0703 16.4355 16.2598C16.4355 13.7402 13.2812 10.2637 8.21289 10.2637C3.1543 10.2637 0 13.7402 0 16.2598C0 17.0703 0.625 17.5586 1.66992 17.5586ZM8.22266 8.51562C10.3125 8.51562 12.1289 6.64062 12.1289 4.19922C12.1289 1.78711 10.3125 0 8.22266 0C6.13281 0 4.31641 1.82617 4.31641 4.21875C4.31641 6.64062 6.12305 8.51562 8.22266 8.51562Z"
                              fill-opacity="0.85"
                            ></path>
                          </g>
                        </svg>
                      </span>
                      <Link href="#">
                        <p className="ml-2 text-sm font-semibold">Profile</p>
                      </Link>
                    </div>
                  </div>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>

      {/* Main Content */}
      <div className="flex flex-1 flex-col w-full overflow-hidden ">
        {/* Top Bar */}
        <header className="flex items-center justify-between w-full h-16 bg-gray-900 p-2 md:p-2 sm:p-1 md:hidden ">
          <div className="flex items-center space-x-2 text-gray-200">
            <button onClick={toggleSideMenu} className="p-1 focus:outline-none">
              <Bars3Icon className="h-6 w-6" />
            </button>
          </div>
        </header>
        {/* Main Content Area */}
        <main className="flex-1 flex justify-center items-center overflow-y-auto m-3 bg-gray-800 rounded-md">
          <Outlet />
        </main>
      </div>
    </div>
  );
};

export default Dashboard;
